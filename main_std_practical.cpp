...
public:
    ...
    zipf_table_distribution(const IntType n, const RealType s = 1.0) : n_(n), s_(s) {
        // 计算概率分布 - 限制大小以避免内存问题
        size_t max_size = std::min(static_cast<size_t>(n), static_cast<size_t>(10000));
        std::vector<RealType> weights(max_size);
        RealType sum = 0.0;
...
public:
    ConcurrentHashTable(size_t num_buckets) : num_buckets_(num_buckets) {
    ...
    void put(const K& key, const V& value) {
        ...
        auto result = buckets_[bucket_idx].map.insert_or_assign(key, value);
...
class PracticalWebServiceBenchmark {
private:
    // 实用的配置参数 - 避免内存分配问题
    ...
    static constexpr uint32_t kNumMutatorThreads = 4; // 减少线程数
    static constexpr double kZipfParamS = 0.85;
    static constexpr uint32_t kNumKeysPerRequest = 8; // 减少每次请求的key数量
    static constexpr uint32_t kReqLen = kKeyLen - 2;
    static constexpr uint32_t kPrintPerIters = 1024;
    static constexpr uint32_t kMaxPrintIntervalUs = 1000 * 1000; // 1 second
    static constexpr uint32_t kPrintTimes = 50; // 减少运行时间
    ...
    std::vector<std::vector<uint32_t>> all_zipf_req_indices;  // 使用uint32_t减少内存
    ...
    void calculate_data_sizes(size_t memory_limit_mb) {
        // 更保守的内存使用策略
        size_t available_mb = memory_limit_mb * 0.6; // 使用60%的内存限制，留更多缓冲
        
        // 限制最大数据量以避免内存分配问题
        size_t max_hashmap_mb = std::min(available_mb * 4 / 10, static_cast<size_t>(2048)); // 最多2GB
        size_t max_array_mb = std::min(available_mb * 6 / 10, static_cast<size_t>(3072));   // 最多3GB
        
        // 计算KV对数量：假设每个KV对平均60字节（包括哈希表开销）
        kNumKVPairs = (max_hashmap_mb * 1024ULL * 1024ULL) / 60;
        
        // 计算数组条目数量：每个条目8KB + 指针开销
        kNumArrayEntries = (max_array_mb * 1024ULL * 1024ULL) / (kArrayEntrySize + 8);
        
        // 计算请求数量
        kNumReqs = kNumKVPairs / kNumKeysPerRequest;
        kReqSeqLen = std::min(kNumReqs, static_cast<uint64_t>(100000)); // 限制序列长度
        
        // 确保最小值和合理上限
        kNumKVPairs = std::max(static_cast<uint64_t>(10000), std::min(kNumKVPairs, static_cast<uint64_t>(50000000)));
        kNumArrayEntries = std::max(static_cast<uint32_t>(100), std::min(kNumArrayEntries, static_cast<uint32_t>(400000)));
        kNumReqs = std::max(static_cast<uint64_t>(1000), kNumReqs);
        
        std::cout << "Practical sizing for " << memory_limit_mb << "MB memory limit:" << std::endl;
        std::cout << "  Hashmap: " << max_hashmap_mb << "MB (" << kNumKVPairs << " KV pairs)" << std::endl;
        std::cout << "  Array: " << max_array_mb << "MB (" << kNumArrayEntries << " entries)" << std::endl;
        std::cout << "  Requests: " << kNumReqs << " (sequence: " << kReqSeqLen << ")" << std::endl;
    }
    
    void append_uint32_to_char_array(uint32_t n, uint32_t suffix_len, char* array) {
        uint32_t len = 0;
        while (n && len < suffix_len) {
            auto digit = n % 10;
            array[len++] = digit + '0';
            n = n / 10;
        }
    ...
    void prepare(HashTable* hashtable) {
        ...
        const size_t batch_size = 10000; // 批次大小
        ...
        for (uint32_t tid = 0; tid < kNumMutatorThreads; tid++) {
            threads.emplace_back([&, tid]() {
                ...
                for (uint64_t batch_start = 0; batch_start < num_reqs_per_thread; batch_start += batch_size) {
                    uint64_t batch_end = std::min(batch_start + batch_size, num_reqs_per_thread);
                    ...
                    if (batch_start % (batch_size * 10) == 0) {
                        std::cout << "Thread " << tid << " progress: " 
                                  << (batch_start * 100 / num_reqs_per_thread) << "%" 
                                  << " (hashtable size: " << hashtable->size() << ")" << std::endl;
                    }
                }
        ...
        std::cout << "Final hashtable size: " << hashtable->size() << std::endl;
        std::cout << "Generating Zipf distribution indices..." << std::endl;
        
        // 生成Zipf分布的请求序列
        all_zipf_req_indices.resize(kNumCPUs);
        zipf_table_distribution<uint32_t> zipf(static_cast<uint32_t>(kNumReqs), kZipfParamS);
    ...
    void consume_array_entry(const ArrayEntry& entry) {
        ...
        stfEncryptor.Put((const unsigned char*)&entry.data, sizeof(entry.data));
...
public:
    PracticalWebServiceBenchmark(size_t memory_limit_mb) : kNumCPUs(std::max(1u, std::thread::hardware_concurrency())) {
    ...
    void run() {
        // 选择合适的哈希表桶数
        size_t num_buckets = std::min(static_cast<size_t>(65536), static_cast<size_t>(kNumKVPairs / 100 + 1));
...
void set_memory_limit_mb(size_t limit_mb) {
    struct rlimit limit;
    limit.rlim_cur = limit_mb * 1024ULL * 1024ULL;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Starting Practical WebService benchmark with " << memory_limit_mb << " MB memory limit" << std::endl;
    ...
    try {
        PracticalWebServiceBenchmark benchmark(memory_limit_mb);
        benchmark.run();
